...
function Lib:CreateWindow(name)
	...
	TopBar.InputBegan:Connect(function(inp)
		if inp.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			windowPos = {X = Window.Position.X.Offset, Y = Window.Position.Y.Offset, Xs = Window.Position.X.Scale, Ys = Window.Position.X.Scale}
			UIDragger = Mouse.Move:Connect(function()
				local newMouse = game:GetService("Players").LocalPlayer:GetMouse()
				Window.Position = UDim2.new(0.5, newMouse.X - (dragPos.X - windowPos.X), 0.5, newMouse.Y - (dragPos.Y - windowPos.Y))
			end)
		end
	end)
	...
	function Win:AddTab(name, istabintab)
		...
		local libinstances = {'LineUnderTabs','TabButtons','TopBar','TabButtonsLayout','TabInTabButtonsLayout','LineUnderTabs_TabInTab'}
		...
		function Tab:AddTabInTab(name)
			...
			TabButton_2:GetPropertyChangedSignal("TextBounds"):Connect(function()
			...
			TabInTabLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
				local pfix = {3,-3,-9}
			...
			TabButton_2.MouseButton1Down:Connect(function()
			...
			function TabInTab:CreateGroupbox(name)
				...
				function GroupBox:AddToggle(name, default, callback)
					...
					function _Toggle:AddColorpicker(default, callback)
						...
						function _Colorpicker:Set(hue,sat,val,nocallback)
							local colorargs = {hue or colorHueStore,sat or colorSaturationStore,val or colorValueStore}
						...
						Colorgradient.InputBegan:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.MouseButton1 then
								HueSaturationInput = Mouse.Move:Connect(function()
									local ColorHue = math.clamp(Mouse.X - Colorgradient.AbsolutePosition.X, 0, Colorgradient.AbsoluteSize.X) / Colorgradient.AbsoluteSize.X
									local ColorSaturation = math.clamp(Mouse.Y - Colorgradient.AbsolutePosition.Y, 0, Colorgradient.AbsoluteSize.Y) / Colorgradient.AbsoluteSize.Y
						...
						Colorgradient.InputEnded:Connect(function(uit)
						...
						local colorValue
						Colorslider.InputBegan:Connect(function(uit)
							if uit.UserInputType == Enum.UserInputType.MouseButton1 then
								colorValue = Mouse.Move:Connect(function()
									local hsvV = math.clamp(Mouse.Y - Colorslider.AbsolutePosition.Y, 0, Colorslider.AbsoluteSize.Y) / Colorslider.AbsoluteSize.Y
						...
						Colorslider.InputEnded:Connect(function(uit)
							if uit.UserInputType == Enum.UserInputType.MouseButton1 then
								if colorValue then
				...
				function GroupBox:AddSlider(name, min, max, default, callback)
					...
					local function MoveSlider(inp)
						local pos = UDim2.new(math.clamp( (inp.X - SliderObject.AbsolutePosition.X) / SliderObject.AbsoluteSize.X, 0, 1 ), 0, 0, 2)
						SliderMover.Position = UDim2.new(math.clamp( (inp.X - SliderObject.AbsolutePosition.X) / SliderObject.AbsoluteSize.X, 0.01, 0.97 ), 0, 0, 2)
						SliderValue = math.clamp(math.floor( (pos.X.Scale * max) / max * ( (max - min) + min ) ), min, max)
						SliderText.Text = tostring(SliderValue)
						callback(SliderValue)
					end

					SliderObject.InputBegan:Connect(function(inp)
						if inp.UserInputType == Enum.UserInputType.MouseButton1 and not getgenv().SlidingColor then
							getgenv().SlidingSlider = true
							game:GetService("RunService"):BindToRenderStep("sliderdrag", 1, function()
				...
				function GroupBox:AddDropdown(name, options, callback)
					local _Dropdown = {}
					...
					ListDDLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
					...
					DropdownObject.InputBegan:Connect(function(inp)
						if inp.UserInputType == Enum.UserInputType.MouseButton1 then
				...
				function GroupBox:AddKeybind(name, callback)
					...
					local bindnotbinded = {--[[Enum.KeyCode.A,Enum.KeyCode.D,Enum.KeyCode.S,Enum.KeyCode.W--]]}
					...
					KeybindButton.MouseButton1Up:Connect(function()
						if not isListening then
							KeybindButton.Text = "..."
							isListening = true
							local InputListener; InputListener = game:GetService("UserInputService").InputBegan:Connect(function(k)
								if not table.find(bindnotbinded, k.KeyCode) and not table.find(bindnotbinded, k.UserInputType) then
									if table.find(bindalways, k.KeyCode) or table.find(bindalways, k.UserInputType) then
										KeybindValue = "NONE"
									...
									local inputtype = tostring(k.KeyCode):find("Enum.KeyCode.") and tostring(k.KeyCode) ~= "Enum.KeyCode.Unknown" and "KeyCode" or "UserInputType"
									KeybindValue = k[inputtype]
									callback(k[inputtype])
									KeybindButton.Text = tostring(k[inputtype]):gsub("Enum.KeyCode.", ""):gsub("Enum.UserInputType.", ""):gsub("Button", "")
									isListening = false
									InputListener:Disconnect()
								end
							end)
					...
					function _Keybind:Set(value)
						...
						KeybindButton.Text = tostring(value):gsub("Enum.KeyCode.", ""):gsub("Enum.UserInputType.", ""):gsub("Button", "")
					end
				...
				function GroupBox:AddButton(name, callback)
                    ...
                    ButtonInstance:GetPropertyChangedSignal("TextBounds"):Connect(function()
				        ButtonInstance.Size = UDim2.new(0, ButtonInstance.TextBounds.X + 10, 0, 16)
			        end)
                    task.wait()
                    ButtonInstance.Size = UDim2.new(0, ButtonInstance.TextBounds.X + 10, 0, 16)
					
					local ButtonConnection; ButtonConnection = ButtonInstance.MouseButton1Down:Connect(function()
						callback()
					end)
                ...
                function GroupBox:AddMenuButton(name)
                    ...
                    MenuButtonInstance:GetPropertyChangedSignal("TextBounds"):Connect(function()
			...
			task.wait()
			Toggle.Size = UDim2.new(0, ToggleLabel.TextBounds.X + 50, 0, 18)
                        ...
                        function _Toggle:AddColorpicker(default, callback)
                            ...
                            local HueSaturationInput
                            Colorgradient.InputBegan:Connect(function(input)
                                if input.UserInputType == Enum.UserInputType.MouseButton1 and Colorpicker.Visible and not getgenv().SlidingSlider then
                            ...
                            local colorValue
                            Colorslider.InputBegan:Connect(function(uit)
                                if uit.UserInputType == Enum.UserInputType.MouseButton1 and Colorpicker.Visible and not getgenv().SlidingSlider then
									getgenv().SlidingColor = true
                                    colorValue = Mouse.Move:Connect(function()
                                        local hsvV = math.clamp(Mouse.Y - Colorslider.AbsolutePosition.Y, 0, Colorslider.AbsoluteSize.Y) / Colorslider.AbsoluteSize.Y
                                        slidebar.Position = UDim2.new(0, 0, hsvV, 0)
					...
					function _MenuButton:AddDropdown(name, options, callback)
						local _Dropdown = {}
